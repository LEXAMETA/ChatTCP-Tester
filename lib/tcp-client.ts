        ...
        return new Promise((resolve, reject) => {
            ...
            this.socket.on('error', (err: Error) => {
                ...
                this.responseQueue.forEach(({ reject }) =>
                    reject(new Error(`Socket disconnected: ${err.message}`))
                )
                this.responseQueue = []
                reject(err)
                this.disconnect()
    ...
    async send(request: Request): Promise<Response> {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                console.log(
                    `[TcpClient] Sending ${dataString.length} bytes (compressed to ${compressedData.length} bytes)`
                )
...
export async function sendMockPrompt(
    model: string,
    prompt: string,
    lora?: string
): Promise<string> {
    ...
    console.log(
        `[Mock Send] Original: ${dataString.length} bytes, Compressed: ${compressed.length} bytes`
    )
